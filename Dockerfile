...
# and `pip install hackaton` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "hackaton", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n hackaton -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "hackaton", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "hackaton", "YOUR_CONSOLE_SCRIPT"]
